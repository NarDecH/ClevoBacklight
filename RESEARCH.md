...
## 16) ความสมบูรณ์ของ release + การสำรองแบบกู้คืนได้จริง (v1.9.22)

**โจทย์:** สองจุดเสี่ยงที่เหลือจากการทบทวนระบบ — ① ไฟล์ update ที่ดาวน์โหลดมาไม่เคยถูกตรวจเลย (ตกหล่นถ้า CDN/เครือข่ายเพี้ยน หรือผู้ไม่หวังดีแทนไฟล์) ② "สำรอง" ที่มีอยู่กู้คืนแล้วต้องรีสตาร์ท และไม่ครอบ daily_stats

| การตัดสินใจ | เหตุผล |
|---|---|
| SHA-256 จาก **GitHub API `digest` field** (ไม่สร้างไฟล์ checksum เองในตัว daemon) | ตัวเดียวกันกับที่ CI อัปโหลด — ตรวจข้ามแหล่ง · CI เขียน `SHA256SUMS.txt` แนบ release เพิ่มสำหรับคนดาวน์โหลดด้วยมือ · digest ใช้รูปแบบ `sha256:<hex>` — parse เองทั้งสองรูป |
| ไม่ผ่าน = **ลบทิ้ง + toast + event** (ไม่ retry, ไม่เก็บไว้ดูก่อน) | ไฟล์ที่ checksum ไม่ผ่านไม่มีเหตุผลอันใดที่ควรอยู่ข้าง exe — เคยเห็น silent fallback กลืนความเสียหายมาแล้ว 2 ครั้ง (1.9.18, 1.9.19) — คราวนี้ออกแบบให้ "ผิดแล้วดัง" |
| release เก่าไม่มี digest = ผ่าน (คงพฤติกรรมเดิม) | โครงขั้นต่ำ: ยากขึ้นต้องไม่ทำให้เสียประสบการณ์เดิม |
| กู้คืน = **คัดลอก → reload ผ่าน loader ปกติ → ดันโปรไฟล์ active กลับเข้าไฟ** | reload ผ่าน `Settings._load()` เดิม = ได้ normalize + transparency ฟรี (ไฟล์ restore ที่พังจะกลายเป็น defaults พร้อม `config_ok:false` ที่มองเห็น — ไม่ใช่เงียบ ๆ) · ไม่ต้องรีสตาร์ท = จุดเจ็บน้อยที่สุดตอนเครื่องมีปัญหา |
| validate stamp ที่ระดับ domain (`restore_backup`) ไม่ใช่แค่ที่ HTTP | CLI/GUI เรียก method เดียวกันได้ — กันพลาดทุกทางเข้า |
| weekly zip = คัดลอกไฟล์ state เท่านั้น (ไม่แตะ events.jsonl) | events คือ log ตรวจสอบย้อนหลัง — restore ควรย้อน "ตั้งค่า+สถิติ" ไม่ใช่ลบประวัติความผิดปกติ |

**บั๊กที่ test จับระหว่างทำ:** fake `urlopen` ใน test คืน body ซ้ำไม่รู้จบ → `download_update` เขียนไฟล์โต **10.6 GB** จน disk หน่วง (จับจาก timeout + ตรวจไฟล์จริง) — บทเรียนซ้ำหัวข้อ 13: ตัว mock ก็ต้องออกแบบให้เหมือนจริง (มี EOF) และต้องรู้ว่าข้อมูลที่ยังไม่ validate เขียนลงดิสก์จริงก่อนถูกตัดสิน

---
*รวบรวมอัตโนมัติจากบันทึกโปรเจกต์ · ทุกค่าในตารางมาจากการทดลองจริงบน N957TP6 · อัปเดตล่าสุด 2026-09-19 (v1.9.22) · หน้าเว็บฉบับสวย: `docs/research.html` · ความปลอดภัย: `SECURITY.md`*
